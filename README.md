...
This repository turns the article walkthrough into an executable, testable, reusable codebase for binary classification threshold tuning, business-impact evaluation, capacity-aware decisioning, segment thresholds, and auditable production decision logging.
...
Every executable Python step from Part 1 is represented in `examples/`. Step 14 is guidance, so it is documented rather than implemented as code.
...
| ROC vs Precision-Recall discussion | `governed_decisions.metrics`, `docs/part-1-threshold-tuning-in-python.md` | Keeps ROC-AUC in the threshold report and explains operating-point selection |
...
| Step 14: Common Anti-Patterns | `docs/article-step-to-code-map.md`, `docs/part-2-enterprise-ai-governance.md` | Documents the operational and governance anti-patterns that should be avoided |
...
3. Run each numbered example to connect the code to the article steps.
4. Review `docs/article-step-to-code-map.md` to see exact coverage.
5. Review the Mermaid diagrams in `assets/` to connect the code to Part 2 governance architecture.
6. Adapt the business value assumptions and capacity limits to your own operating context.
